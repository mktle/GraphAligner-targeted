...
void MinimizerSeeder::addMinimizers(std::vector<SeedHit>& result, std::vector<std::tuple<size_t, size_t, size_t, size_t>>& matchIndices, size_t maxCount, size_t filterStart, size_t filterEnd) const
{
    ...
    for (auto match : matchIndices)
    {
        ...
        for (size_t i = start; i < end; i++)
        {
            ...
            if (offset + graph.NodeOffset((size_t)nodeId) >= filterStart && offset + graph.NodeOffset((size_t)nodeId) <= filterEnd)
            {
...
std::vector<SeedHit> MinimizerSeeder::getSeeds(const std::string& sequence,const std::string& seqName, double density) const
{
    ...
    // retrieve targeted region from sequence name
    size_t lastHyphen = seqName.find_last_of('-');
    size_t secondLastHyphen = seqName.rfind('-', lastHyphen - 1);
    size_t filterStart = std::stoi(seqName.substr(secondLastHyphen + 1, lastHyphen - secondLastHyphen - 1));
    size_t filterEnd = std::stoi(seqName.substr(lastHyphen + 1));

    std::vector<SeedHit> result;
    size_t maxHits = sequence.size() * density;
    if (density == -1) maxHits = std::numeric_limits<size_t>::max();
    addMinimizers(result, matchIndices, maxHits, filterStart, filterEnd);
    return result;
}
