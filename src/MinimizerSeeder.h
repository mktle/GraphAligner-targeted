...
public:
    ...
    std::vector<SeedHit> getSeeds(const std::string& sequence, const std::string& seqName, double density) const;
    bool canSeed() const;
private:
    void addMinimizers(std::vector<SeedHit>& result, std::vector<std::tuple<size_t, size_t, size_t, size_t>>& matchIndices, size_t maxCount, size_t filterStart, size_t filterEnd) const;
